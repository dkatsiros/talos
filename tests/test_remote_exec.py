"""Tests for the remote-offload execution_host targeting (Path C).
...
mandated scenarios:
  ...
  (b) fallback    — preflight fails, the local screen path runs instead and one
      JSON line is appended to logs/remote-offload-fallback.jsonl.
"""
...
# Use the module's own default so the test stays correct after env-var changes.
MIRROR_ROOT = remote_exec.DEFAULT_MIRROR_ROOT
...
def _prepare_remote_task(tmp_path: Path) -> tuple[Path, str]:
    ...
    envelope["execution_host"] = "remote"
...
def test_happy_path_wraps_dispatch_in_ssh_screen(tmp_path, monkeypatch):
    project, task = _prepare_remote_task(tmp_path)
    ...
    fallback_log = project / ".openclaw" / "claude-loop" / "logs" / "remote-offload-fallback.jsonl"
...
def test_remote_config_overrides():
    cfg = remote_exec.remote_config(
        {"remote": {"ssh_target": "u@host", "mirror_root": "/m", "preflight_timeout_s": 7}}
    )
    assert cfg == {"ssh_target": "u@host", "mirror_root": "/m", "preflight_timeout_s": 7}


# --- injection-guard tests (Fix F1) ---


def test_remote_config_rejects_injected_ssh_target():
    """ssh_target starting with '-' must be rejected (option injection)."""
    with pytest.raises(ValueError, match="ssh_target"):
        remote_exec.remote_config({"remote": {"ssh_target": "-oProxyCommand=evil", "mirror_root": "/m"}})


def test_remote_config_rejects_bare_hostname_ssh_target():
    """ssh_target without @ must be rejected (ambiguous / not user@host)."""
    with pytest.raises(ValueError, match="ssh_target"):
        remote_exec.remote_config({"remote": {"ssh_target": "hostname-only", "mirror_root": "/m"}})


def test_remote_config_rejects_relative_mirror_root():
    """mirror_root that is not an absolute path must be rejected."""
    with pytest.raises(ValueError, match="mirror_root"):
        remote_exec.remote_config({"remote": {"ssh_target": "u@host", "mirror_root": "relative/path"}})


def test_remote_config_rejects_dash_mirror_root():
    """mirror_root starting with '-' must be rejected (rsync option injection)."""
    with pytest.raises(ValueError, match="mirror_root"):
        remote_exec.remote_config({"remote": {"ssh_target": "u@host", "mirror_root": "-some-rsync-flag"}})
...
def test_remote_inner_carries_default_model():
    ...
    assert "export ANTHROPIC_MODEL=claude-opus-5 &&" in inner
    # and it survives the ssh/screen quoting layers
    argv = remote_exec.build_remote_screen_argv("u@host", "claude-loop-t123", inner)
    assert "ANTHROPIC_MODEL=claude-opus-5" in argv[-1]
...
def test_resolve_anthropic_model_reads_process_env(monkeypatch):
    ...
    assert remote_exec.resolve_anthropic_model() == "claude-opus-5"
...
def test_log_fallback_appends_one_line(tmp_path):
    ...
    lines = (logs / "remote-offload-fallback.jsonl").read_text(encoding="utf-8").splitlines()
    assert len(lines) == 2
